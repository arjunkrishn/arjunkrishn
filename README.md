<div align="center">

<img src="https://capsule-render.vercel.app/api?type=rect&color=0D1117&height=200&section=header&text=Arjun%20Krishna%20B&fontSize=48&fontColor=39FF14&animation=fadeIn&fontAlignY=38&desc=SOC%20Analyst%20%7C%20Cybersecurity%20Analyst%20%7C%20Network%20Security&descAlignY=58&descSize=18&descColor=39FF14" width="100%"/>

<img src="https://readme-typing-svg.demolab.com/?font=Fira+Code&size=20&duration=3000&pause=1000&color=39FF14&background=0D1117&center=true&vCenter=true&width=650&lines=%24+whoami;%3E+Cybersecurity+Graduate+%7C+SOC+%2F+Network+Security;%24+cat+skills.txt;%3E+FortiGate+%7C+IPSec+VPN+%7C+TCP-IP+%7C+Python;%24+echo+%24STATUS;%3E+Open+to+SOC+%26+Cybersecurity+Analyst+roles" alt="Typing SVG" />

<br/>

![Degree](https://img.shields.io/badge/BCA-AIML-39FF14?style=flat-square&labelColor=0D1117)
![University](https://img.shields.io/badge/VIT-Vellore-39FF14?style=flat-square&labelColor=0D1117)
![Location](https://img.shields.io/badge/Location-Cherthala,%20India-39FF14?style=flat-square&labelColor=0D1117)

<br/>

[![Portfolio](https://img.shields.io/badge/Portfolio-0D1117?style=for-the-badge&logo=vercel&logoColor=39FF14&color=0D1117)](https://arjunportfolio.super.site)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=39FF14&color=0D1117)](https://linkedin.com/in/arjunn100)
[![Email](https://img.shields.io/badge/Email-0D1117?style=for-the-badge&logo=gmail&logoColor=39FF14&color=0D1117)](mailto:arjunkb45@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=39FF14&color=0D1117)](https://github.com/arjunkrishn)

<br/>

![Profile Views](https://komarev.com/ghpvc/?username=arjunkrishn&color=39FF14&style=flat-square&label=PROFILE+VIEWS)
![Followers](https://img.shields.io/github/followers/arjunkrishn?style=flat-square&color=39FF14&labelColor=0D1117)
![Stars](https://img.shields.io/github/stars/arjunkrishn?style=flat-square&color=39FF14&labelColor=0D1117)

</div>

---

### `$ cat about.md`

Cybersecurity graduate with hands-on experience configuring FortiGate firewalls, IPSec VPNs, firewall policies, NAT, and VLANs during a Network Security internship. Strong foundation in TCP/IP, endpoint protection, and threat management, backed by four independent security/ML projects. Currently seeking an entry-level **SOC Analyst** or **Cybersecurity Analyst** role.

```bash
ROLE      : Cybersecurity Graduate
DOMAIN    : Network Security | SOC
STACK     : FortiGate, Python, Linux, Wireshark, Nmap, TCP/IP
OPEN_TO   : SOC Analyst, Cybersecurity Analyst
```

---

### `$ ls tech-stack/`

<div align="center">
<img src="https://skillicons.dev/icons?i=py,linux,git,github,bash&theme=dark" />
</div>

<div align="center">

![FortiGate](https://img.shields.io/badge/FortiGate-39FF14?style=for-the-badge&labelColor=0D1117&color=0D1117&logoColor=39FF14)
![Wireshark](https://img.shields.io/badge/Wireshark-0D1117?style=for-the-badge&labelColor=0D1117&color=0D1117&logoColor=39FF14)
![Nmap](https://img.shields.io/badge/Nmap-0D1117?style=for-the-badge&labelColor=0D1117&color=0D1117&logoColor=39FF14)
![TCP/IP](https://img.shields.io/badge/TCP%2FIP-0D1117?style=for-the-badge&labelColor=0D1117&color=0D1117&logoColor=39FF14)
![IPSec VPN](https://img.shields.io/badge/IPSec%20VPN-0D1117?style=for-the-badge&labelColor=0D1117&color=0D1117&logoColor=39FF14)
![VLAN](https://img.shields.io/badge/VLAN-0D1117?style=for-the-badge&labelColor=0D1117&color=0D1117&logoColor=39FF14)
![NAT](https://img.shields.io/badge/NAT-0D1117?style=for-the-badge&labelColor=0D1117&color=0D1117&logoColor=39FF14)

</div>

---

### `$ cat specialties.txt`

<div align="center">

![Network Security](https://img.shields.io/badge/Network%20Security-39FF14?style=flat-square&labelColor=0D1117)
![SOC Fundamentals](https://img.shields.io/badge/SOC%20Fundamentals-39FF14?style=flat-square&labelColor=0D1117)
![Threat Detection](https://img.shields.io/badge/Threat%20Detection-39FF14?style=flat-square&labelColor=0D1117)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-39FF14?style=flat-square&labelColor=0D1117)
![NLP](https://img.shields.io/badge/NLP-39FF14?style=flat-square&labelColor=0D1117)

</div>

---

### `$ cat expertise.md`

| Domain | Proficiency | Details |
|---|:---:|---|
| Firewall Management | ●●●●○ | FortiGate: policies, NAT, VLANs, IPS, Web Filtering, Antivirus, App Control |
| VPN / IPSec | ●●●●○ | Configured site-to-site IPSec VPNs for secure enterprise communication |
| Network Traffic Analysis | ●●●○○ | Packet inspection & protocol analysis (TCP/UDP/ICMP/DNS/HTTP) with Scapy & Wireshark |
| Python for Security | ●●●●○ | Vulnerability scanning, traffic analysis, threat detection tooling |
| Machine Learning / NLP | ●●●○○ | TF-IDF, CountVectorizer, classification models for fraud/threat detection |
| Vulnerability Assessment | ●●●○○ | Host discovery, port scanning, service enumeration with Nmap |

---

### `$ ls projects/ --featured`

<details open>
<summary><b>🛡️ Vulnerability Scanner using Python</b></summary>
<br/>

Python-based vulnerability scanner for identifying security weaknesses and exposed services on target systems — automated host discovery, port scanning, service enumeration, and misconfiguration checks, with detailed scan reports for risk assessment.

| Stack | Scale | Impact |
|---|---|---|
| Python, Nmap, Python-Nmap, Sockets, Threading, JSON, CSV | Multi-host scans | Automated vulnerability & risk reporting |

</details>

<details>
<summary><b>🤖 AI Threat Detection System</b></summary>
<br/>

Python-based system automating identification and classification of cybersecurity threats from network traffic and security data — data preprocessing, anomaly detection, and ML-based classification of suspicious/malicious activity.

| Stack | Scale | Impact |
|---|---|---|
| Python, Scikit-learn, Pandas, NumPy, Streamlit | Real-time alerting | Proactive threat monitoring & incident response support |

</details>

<details>
<summary><b>📡 Network Traffic Analyzer using Python</b></summary>
<br/>

Real-time network traffic monitor with packet inspection and protocol analysis, identifying TCP/UDP/ICMP/DNS/HTTP patterns and generating traffic statistics for anomaly detection.

| Stack | Scale | Impact |
|---|---|---|
| Python, Scapy, Pandas, Matplotlib, Wireshark | Real-time capture | Traffic statistics & anomaly detection reporting |

</details>

<details>
<summary><b>🕵️ Fake Job Posting Detection (ML + NLP)</b></summary>
<br/>

Fraud detection system classifying job postings as genuine or fraudulent using NLP and ML — text preprocessing, TF-IDF/CountVectorizer feature extraction, and classification model evaluation.

| Stack | Scale | Impact |
|---|---|---|
| Python, Scikit-learn, NLTK, Pandas, Matplotlib, Seaborn | Full ML pipeline | Detects fraudulent recruitment scam postings |

</details>

---

### `$ git log --experience`

**Network Security Intern** · Secure Solutions · `07/2026 – 08/2026` · Kochi
- Installed, configured, and managed FortiGate firewalls to secure enterprise network infrastructure
- Configured firewall policies, NAT, VLANs, and IPsec VPNs for secure network communication
- Implemented IPS, Web Filtering, Antivirus, and Application Control security features
- Monitored firewall logs, analyzed security events, and applied network security best practices

`FortiGate` `IPSec VPN` `NAT` `VLAN` `IPS` `Log Monitoring`

<br/>

**Web UI Development Intern** · Techgentsia Software Technologies Pvt. Ltd. · `06/2025 – 06/2025` · Infopark Pallipuram
- Assisted in developing and implementing responsive web user interfaces
- Collaborated with development teams to support front-end design and testing activities

`Front-End` `Responsive Design` `Testing`

---

### `$ cat achievements.md`

<div align="center">

| Achievement | Detail |
|---|---|
| 🎓 CGPA | 7.67 / 10 — BCA (AIML), VIT Vellore |
| 📜 Certifications Earned | 7 (Cybersecurity, Cloud, AI foundations — see below) |
| 🧪 Independent Security Projects | 4 built end-to-end |
| 🌐 Languages | English, Tamil, Hindi, Malayalam |

</div>

---

### `$ cat education.md`

<div align="center">

[![Education](https://img.shields.io/badge/BCA%20(AIML)%20—%20Vellore%20Institute%20of%20Technology-39FF14?style=for-the-badge&labelColor=0D1117)](#)

`08/2023 – 04/2026` · Vellore · CGPA 7.67/10

</div>

---

### `$ cat certifications.md`

- Junior Cybersecurity Analyst Career Path
- Endpoint Security — Oracle Cloud Infrastructure
- Certified AI Foundations Associate
- Getting Started with Cybersecurity — IBM
- Network Support and Security — Oracle Cloud Infrastructure
- Certified Foundations Associate — Cyber Threat Management
- Oracle AI Database Certified Foundations Associate

---

### `$ ./analytics.sh`

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=arjunkrishn&show_icons=true&bg_color=0D1117&title_color=39FF14&text_color=C9D1D9&icon_color=39FF14&border_color=39FF14&hide_border=false" width="49%" />
<img src="https://streak-stats.demolab.com/?user=arjunkrishn&background=0D1117&ring=39FF14&fire=39FF14&currStreakLabel=39FF14&sideLabels=C9D1D9&currStreakNum=C9D1D9&sideNums=C9D1D9&dates=C9D1D9&border=39FF14&hide_border=false" width="49%" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=arjunkrishn&layout=compact&bg_color=0D1117&title_color=39FF14&text_color=C9D1D9&border_color=39FF14&hide_border=false" width="49%" />

</div>

---

### `$ ./trophies.sh`

<div align="center">
<img src="https://github-profile-trophy.vercel.app/?username=arjunkrishn&theme=onedark&row=1&column=7&title=39FF14&rank=39FF14&border=39FF14&background=0D1117&no-frame=true" />
</div>

---

### `$ ./activity-graph.sh`

<div align="center">
<img src="https://github-readme-activity-graph.vercel.app/graph?username=arjunkrishn&bg_color=0D1117&color=39FF14&line=39FF14&point=C9D1D9&area=true&area_color=39FF14&hide_border=true" width="100%" />
</div>

---

### `$ ./summary-cards.sh`

<div align="center">
<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details.svg?username=arjunkrishn&theme=github_dark" width="100%" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language.svg?username=arjunkrishn&theme=github_dark" width="49%" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language.svg?username=arjunkrishn&theme=github_dark" width="49%" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/stats.svg?username=arjunkrishn&theme=github_dark" width="49%" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time.svg?username=arjunkrishn&theme=github_dark&utcOffset=5.5" width="49%" />
</div>

> **Note:** `github-profile-summary-cards` only ships fixed themes (no custom hex) — `github_dark` used as the closest match to the Terminal Green palette.

---

### `$ ./snake.sh`

<div align="center">
<img src="https://raw.githubusercontent.com/arjunkrishn/arjunkrishn/output/github-contribution-grid-snake-dark.svg" width="100%" />
</div>

> Requires the `platane/snk` GitHub Action enabled on the `arjunkrishn/arjunkrishn` repo — workflow file provided below.

---

### `$ cat current-focus.yaml`

```yaml
currently:
  building:
    - AI Threat Detection System (Python, Scikit-learn, Streamlit)
    - Network Traffic Analyzer (Scapy, protocol analysis)
  learning:
    - Advanced SOC workflows & SIEM fundamentals
    - Deeper IPSec / VPN architecture
  exploring:
    - ML applications in threat detection
  open_to:
    - SOC Analyst
    - Cybersecurity Analyst
```

---

### `$ ./connect.sh`

<div align="center">

[![Portfolio](https://img.shields.io/badge/Portfolio-39FF14?style=for-the-badge&labelColor=0D1117&color=0D1117&logoColor=39FF14)](https://arjunportfolio.super.site)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-39FF14?style=for-the-badge&labelColor=0D1117&color=0D1117&logo=linkedin&logoColor=39FF14)](https://linkedin.com/in/arjunn100)
[![Email](https://img.shields.io/badge/Email-39FF14?style=for-the-badge&labelColor=0D1117&color=0D1117&logo=gmail&logoColor=39FF14)](mailto:arjunkb45@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-39FF14?style=for-the-badge&labelColor=0D1117&color=0D1117&logo=github&logoColor=39FF14)](https://github.com/arjunkrishn)

<br/>

*"Every packet tells a story — I read the ones that matter."*

<img src="https://capsule-render.vercel.app/api?type=waving&color=0D1117&height=120&section=footer" width="100%"/>

</div>
